...
?>
    ...
    <form action="pro_upd_backend.php"  method="POST" enctype="multipart/form-data">
        ...
        <select name="pro_sub_" class="h-20 w-96 border-2  rounded-lg m-8 focus:outline-blue-400 p-4">
        <option value="<?php echo "".$row['pro_sub_cat']."";?>"><?php echo "".$row['pro_sub_cat']."";?>"</option>
        <?php 
            include 'db.php';
    
            $que="SELECT * FROM sub_cat ";
            $result1=$conn->query($que);

            if($result1->num_rows>0)
            {  while($row1=$result->fetch_assoc())
                {
                    $subcat=$row1['sub_category'];
                    $cat=$row1['category'];
                    if($cat=='women')
                    {  
